from unittest import TestCase
from yoloapnea.Plotters.pyPlotter import PyPlotter
import numpy as np
import cv2

image_duration = 900
overlap = 450

class TestPyPlotter(TestCase):

    def setUp(self):
        test_signal = np.load("shhs1-200753-signal.npz")
        self.abdo_signal = test_signal["abdo_res"]

        self.plotter = PyPlotter(image_duration, overlap)


    def test_plot_signal_correct_length(self):
        part_signal = self.abdo_signal[0:image_duration]
        images = self.plotter.plot_signal(part_signal)
        for start_index, img in images:
            shape = img.shape
            self.assertEqual(shape[0],1000)
            self.assertEqual(shape[1],1000)
            self.assertEqual(shape[2],3)

        # cv2.imshow('image', img)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()


    def test_plot_signal_two_images(self):
        part_signal = self.abdo_signal[0:image_duration+overlap]
        images = self.plotter.plot_signal(part_signal)

        image_count = 0

        for _,img in images:
            shape = img.shape
            self.assertEqual(shape[0], 1000)
            self.assertEqual(shape[1], 1000)
            self.assertEqual(shape[2], 3)
            image_count += 1

            # cv2.imshow('image', img)
            # cv2.waitKey(0)
            # cv2.destroyAllWindows()

        self.assertEqual(2,image_count)


    def test_plot_signal_long_data(self):

        import time
        import statistics

        part_signal = self.abdo_signal[:100000]
        images = self.plotter.plot_signal(part_signal)

        times = []
        start = time.time()
        for _,img in images:

            end = time.time()
            duration = end-start
            print(f"Time: {duration:.2}")
            times.append(duration)
            start = time.time()



        print(f"Mean time is {statistics.mean(times):.4}")


        # Orignial Mean time is 0.02398 Mean time is 0.02276
        # New      Mean time is 0.05698
        #Mean time is 0.02107